...
export fn rfsCreate(in: ?*const vs.Map, out: ?*vs.Map, user_data: ?*anyopaque, core: ?*vs.Core, vsapi: ?*const vs.API) callconv(.C) void {
    _ = user_data;
    var d: RFSData = undefined;
    var node_err: c_int = undefined;
    ...
    if ((ne > 0) and (np > 1)) {
        ...
        while (ci < ne) : (ci += 1) {
            ...
            if ((e < 0) or (e >= np)) {
                vsapi.?.mapSetError.?(out, "RFS: plane index out of range.");
                vsapi.?.freeNode.?(d.node1);
                vsapi.?.freeNode.?(d.node2);
                return;
            }
    ...
    vsapi.?.createVideoFilter.?(out, "RFS", &vi, rfsGetFrame, rfsFree, fm.Parallel, &deps, deps.len, data, core);
}
...
fn rfsValidateInput(out: *vs.Map, node1: *vs.Node, node2: *vs.Node, outvi: *vs.VideoInfo, mismatch: bool, vsapi: *const vs.API) rfsInputError!void {
    ...
    if ((outvi.width != vi2.width) or (outvi.height != vi2.height)) {
        if (mismatch) {
            ...
            err_msg = "RFS: Clip dimensions don't match, enable mismatch if you want variable format.";
    ...
    if (!vsh.isSameVideoFormat(&outvi.format, &vi2.format)) {
        if (mismatch) {
            ...
            err_msg = "RFS: Clip formats don't match, enable mismatch if you want variable format.";
    ...
    if ((outvi.fpsDen != vi2.fpsDen) or (outvi.fpsNum != vi2.fpsNum)) {
        if (mismatch) {
            ...
            err_msg = "RFS: Clip frame rates don't match, enable mismatch if you want variable format.";
...
export fn VapourSynthPluginInit2(plugin: *vs.Plugin, vspapi: *const vs.PLUGINAPI) void {
    ...
    _ = vspapi.registerFunction.?("RFS", "clip_a:vnode;clip_b:vnode;frames:int[];mismatch:int:opt;planes:int[]:opt;", "clip:vnode;", rfsCreate, null, plugin);
}
