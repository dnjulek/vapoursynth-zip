...
pub inline fn process(
    ...
    inline for (0..256) |i| {
        const x: f32 = @min(@max(@mulAdd(f32, @exp(scaling2 * ranges[i]), peak, 0.5), 0), peak);
        lut[i] = @intFromFloat(x);
    }
